...
body #magicPointer {
  ...
  body #magicPointer.pointer-overlay {
    box-shadow: 0px 0px 15px -5px white; }
  body #magicPointer.pointer-overlay + #magicMouseCursor {
    transform: var(--transform-matrix) scale(1); }
body .magic-hover {
  transition: all 0.2s; }
...
const magicMouse = (options) => {
  ...
  if (!Modernizr.touchevents){
    ...
    var movement = () => {
      if (cursorDOM) {
        cursorDOM.style.setProperty("--transform-matrix", "matrix(1, 0, 0, 1, " + outerX + ", " + outerY + ")");
